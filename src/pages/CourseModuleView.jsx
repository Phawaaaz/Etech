...
export default function CourseModuleView() {
  ...
  const generationMessages = [
    ...
    "Fleshing out concept definitions and mechansims...",
  ...
  if (loading || generating) {
    return (
      <div className="min-h-screen bg-[#040814] text-white flex flex-col items-center justify-center font-sans p-6 text-center">
        {generating ? (
          <div className="w-full max-w-xl flex flex-col items-center">
            ...
            <h2 className="text-3xl font-bold text-white mb-2 animate-pulse">Assembling Section Materials</h2>
            <p className="text-zinc-400 text-lg font-medium italic min-h-[40px]">{genMessage}</p>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-zinc-400 font-medium select-none">Fetching course section details...</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans flex flex-col w-full">
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-6 md:px-8 py-8 flex flex-col text-left text-zinc-850">
          ...
          <div className="flex flex-col gap-4">
            {section.concepts.map((concept, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="border border-zinc-200 rounded-2xl overflow-hidden bg-zinc-50 shadow-sm"
                >
                  {/* Accordion header */}
                  <button
                    onClick={() => setActiveConceptIdx(isActive ? -1 : idx)}
                    className="w-full px-6 py-4 flex justify-between items-center text-left hover:bg-zinc-100 transition-colors font-bold text-lg select-none"
                  >
                  ...
                  {/* Accordion body */}
                  {isActive && (
                    <div className="p-6 bg-white border-t border-zinc-200 select-text">
        ...
        {section.workedExample && (
          <div className="bg-zinc-900 text-white rounded-3xl p-6 md:p-8 mb-10 shadow-xl select-text">
            <div className="flex items-center gap-2 text-indigo-400 font-extrabold text-xs uppercase tracking-wider mb-4">
            ...
            <ol className="list-decimal pl-5 space-y-2.5 text-zinc-250 text-sm md:text-base mb-6 leading-relaxed">
            ...
            {section.workedExample.expectedOutput && (
              <div className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800/60 text-sm">
                <span className="font-bold text-indigo-400 block mb-1 uppercase tracking-wider text-2xs">Expected Output:</span>
                <p className="font-medium text-zinc-350">{section.workedExample.expectedOutput}</p>
              </div>
            )}
          </div>
        )}
        ...
        {section.commonMistakes && section.commonMistakes.length > 0 && (
          <div className="border border-rose-200 bg-rose-50/20 rounded-3xl p-6 md:p-8 mb-10 shadow-sm select-text">
        ...
        {section.furtherReading && section.furtherReading.length > 0 && (
          <div className="bg-zinc-50 border border-zinc-200 rounded-3xl p-6 md:p-8 mb-12 select-text">
        ...
        {/* Footer Navigation */}
        <div className="flex flex-wrap justify-between items-center gap-4 border-t pt-6 select-none shrink-0">
          <button
            onClick={() => navigate(`/course-index/${courseId}`)}
            className="px-6 py-2.5 border border-zinc-300 rounded-full font-bold text-zinc-600 hover:text-zinc-800 hover:bg-zinc-50 active:scale-95 transition"
          >
          ...
          <div className="flex gap-4">
            <button
              ...
              className="px-6 py-2.5 bg-zinc-950 text-white rounded-full font-bold hover:bg-zinc-900 active:scale-95 transition shadow-md"
            >
              Mark as Read
            </button>
            ...
            <button
              ...
              className="px-8 py-2.5 bg-indigo-600 text-white rounded-full font-bold hover:bg-indigo-700 active:scale-95 transition shadow-md"
            >
              Take Quiz
            </button>
          </div>

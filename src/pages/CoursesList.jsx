...
export default function CoursesList() {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className="w-full min-h-screen bg-background text-foreground p-4 md:p-8 flex flex-col font-sans select-text"
    >
      <div className="max-w-6xl w-full mx-auto text-left">
        {/* Header Section */}
        <motion.div
          variants={itemVariants}
          className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-border pb-6"
        >
          <div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="bg-primary hover:bg-primary/95 text-primary-foreground font-bold px-6 py-3 rounded-full text-sm hover:shadow-md flex items-center gap-1.5 transition-all duration-250 cursor-pointer border-none outline-none"
          >
          ...
          {loading ? (
            ...
            <div className="bg-card border border-border rounded-2xl p-12 text-center shadow-sm mt-8">
              ...
              >
                <Link
                  to="/create-course"
                  className="inline-block px-8 py-3.5 bg-primary text-primary-foreground font-bold rounded-full shadow hover:bg-primary/95 transition-all duration-200"
                >
                  Start Generating
                </Link>
              </motion.div>
            ...
            >
              {courses.map((course) => (
                <motion.div
                  ...
                  className="bg-card border border-border rounded-2xl p-5 shadow-sm cursor-pointer flex flex-col justify-between h-48 group relative overflow-hidden"
                >
                  <div>
